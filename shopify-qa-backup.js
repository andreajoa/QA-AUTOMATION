...
class ShopifyQAAutomation {
  ...
  async runFullQA() {
    ...
    this.log('info', `🏪 LOJA CONECTADA: ${CONFIG.SHOPIFY_STORE.name}`, CONFIG.SHOPIFY_STORE.url);
    this.log('info', '📋 INICIANDO BATERIA DE TESTES', 'Conectividade • APIs • Performance • E2E • Segurança');
    
    const results = {
      store: CONFIG.SHOPIFY_STORE.name,
  ...
  async testConnectivity() {
    ...
    try {
      ...
      this.log('info', `📡 TARGET URL: ${CONFIG.SHOPIFY_STORE.url}`, 'Aguardando resposta...');
      
      const response = await fetch(CONFIG.SHOPIFY_STORE.url, {
        timeout: 10000,
        headers: {
  ...
  async testShopifyAPIs() {
    ...
    try {
      ...
      const productsResponse = await fetch(`${CONFIG.SHOPIFY_STORE.url}/products.json?limit=10`);
      ...
      const collectionsResponse = await fetch(`${CONFIG.SHOPIFY_STORE.url}/collections.json`);
      ...
      const cartResponse = await fetch(`${CONFIG.SHOPIFY_STORE.url}/cart.json`);
  ...
  async testE2EFlow() {
    ...
    try {
      ...
      await page.goto(CONFIG.SHOPIFY_STORE.url, { waitUntil: 'networkidle' });
      ...
      // Tentar encontrar e interagir com produtos
      ...
      try {
        ...
        if (productLink) {
          ...
          this.log('warning', '⚠️ NENHUM LINK DE PRODUTO ENCONTRADO', 'Produtos podem não estar linkados');
        }
      } catch (error) {
  ...
  async testSecurity() {
    ...
    try {
      ...
      const isHTTPS = CONFIG.SHOPIFY_STORE.url.startsWith('https://');
      ...
      res.send(`
        ...
        <body>
            <div class="header">
                ...
                <p style="margin: 0; opacity: 0.9;">Store: ${CONFIG.SHOPIFY_STORE.name}</p>
            </div>
            ...
            <div class="container">
                <div class="logs-container">
                    ...
                    <div class="logs-content" id="logsContainer">
                        ...
                        <div class="log log-info">[0.0s] 🏪 Loja configurada: ${CONFIG.SHOPIFY_STORE.name}</div>
                        <div class="log log-info">[0.0s] 🌐 Target URL: ${CONFIG.SHOPIFY_STORE.url}</div>
...
async function main() {
  ...
  console.log(chalk.white(`🏪 Store: ${CONFIG.SHOPIFY_STORE.name}`));
  console.log(chalk.white(`🌐 URL: ${CONFIG.SHOPIFY_STORE.url}`));
  console.log(chalk.cyan('=====================================\n'));
